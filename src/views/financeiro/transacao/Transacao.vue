...
export default {
  setup() {
    ...
    title = ` ${languagePageNow.add} ${languagePageNow.transacao}`;
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/transacao/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let descricaoItemSelected = e.item.dsTransacao;
        Swal.fire({
          icon: "question",
