<template>
  <div class="d-flex flex-column">
    <a
      ...
      href="/#/portador/novo"
      title="Adicionar novo Portador"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("portador") }}    
    </a>
  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/portador/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        let descricaoItemSelected = e.item.nomePortador;
        Swal.fire({
          icon: "question",
