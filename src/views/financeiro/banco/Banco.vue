<template>
  <div class="d-flex flex-column">
    <a
      ...
      :title="title"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("banco") }}
    </a>
  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        console.log("Editar");
        router.push("/banco/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;

        console.log(e.item);
        let descricaoItemSelected = e.item.nomeBanco;
        Swal.fire({
          icon: "question",
    ...
    onMounted(async () => {
      ...
      const onDelete = async item => {
        try {
          ...
          console.error("Erro ao buscar itens cadastros!", err);
        }
      };
