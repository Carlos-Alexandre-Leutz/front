  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/tipoContato/editar/" + e.item.id);
      }
      ...
      if (e.target.dataset.edit == 2) {
        console.log("Editar", e.item.id);
        router.push("/tipoContato/editar/" + e.item.id + "/visualizar");
      }
    };
    ...
    const searchTab = () => {
      let abasSessionStorage = sessionStorage.getItem("abasAbertas");
      let array = JSON.parse(abasSessionStorage);
