  ...
  <div class="card shadow">
    ...
    <div v-if="componentePaginacao != undefined" class="card-body">
      <suspense>
        ...
        <template #fallback>
          {{ $i18n.translate("carregandoTabela") }}</template
        >
      </suspense>
    </div>
  </div>
...
export default {
  setup() {
    ...
    title = ` ${languagePageNow.add} ${languagePageNow.especificacaoCcapsula}`;
    ...
    const onEdit = e => {  
      ...
      if (e.target.dataset.edit == 1) {
        router.push("/capsula/editar/" + e.item.id);
      }
    ...
    onMounted(async () => {
      ...
      const onDelete = async item => {
        try {
          ...
          console.error("Erro ao buscar dcbs!", err);
        }
      };
