...
export default {
  setup() {
    ...
    const atualizartraducaoParametros = () => {
      ...
      title = ` ${languagePageNow.add} ${languagePageNow.prescritor}`;
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/prescritor/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 2) {
        console.log("Editar", e.item.id);
        router.push("/prescritor/visualizar/" + e.item.id + "/visualizar");
      }
