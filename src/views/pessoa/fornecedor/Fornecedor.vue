...
export default {
  setup() {
    ...
    const atualizartraducaoParametros = () => {
      ...
      title = ` ${languagePageNow.add} ${languagePageNow.fornecedor}`;
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/pessoa/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 2) {
        console.log("Editar", e.item.id);
        router.push("/pessoa/visualizar/" + e.item.id + "/visualizar");
      }
