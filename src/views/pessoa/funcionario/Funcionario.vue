...
export default {
  setup() {
    ...
    const atualizartraducaoParametros = () => {
      ...
      title = ` ${languagePageNow.add} ${languagePageNow.funcionario}`;
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/funcionario/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 2) {
        console.log("Editar", e.item.id);
        router.push("/funcionario/visualizar/" + e.item.id + "/visualizar");
      }
