<template>
  <div class="d-flex flex-column">
    <a
      ...
      :title="title"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("visitador") }}
    </a>
...
export default {
  setup() {
    ...
    let hColumns = ref([
      ...
      {
        key: "comissaoVendedor",
        label: "comissaoVendedor",
      },
    ]);
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        router.push("/visitador/editar/" + e.item.id);
      };
      if (e.target.dataset.edit == 2) {
        router.push("/visitador/visualizar/" + e.item.id + "/visualizar");
      };
