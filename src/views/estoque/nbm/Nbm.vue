<template>
  <div class="d-flex flex-column">
    <a
      ...
      href="/#/nbm/novo"
      title="Adicionar novo Nbm"
    >
      {{ $i18n.translate("adicionar") }}  {{ $i18n.translate("nbm") }}
    </a>
  </div>
  <div class="card shadow">
    ...
    <div v-if="componentePaginacao != undefined" class="card-body">
      <suspense>
        ...
        <template #fallback>{{ $i18n.translate("carregandoTabela") }}</template>
      </suspense>
    </div>
  </div>
</template>
...
export default {
  setup() {
    ...
    let hColumns = ref([
      ...
      {
        key: "aliquotaAgregadoEstadual",
        label: "aliquotaAgregadoEstadual",
      },
      {
        key: "aliquotaAgregadoInterestadual",
        label: "aliquotaAgregadoInterestadual",
      },
      {
        key: "aliquotaComplementarEstadual",
        label: "aliquotaComplementarEstadual",
      },
      {
        key: "aliquotaComplementarInterestadual",
        label: "aliquotaComplementarInterestadual",
      },
    ]);
   ...
   const lang = () => {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/nbm/editar/" + e.item.id);
      }
    ...
    onMounted(async () => {
      ...
      const onDelete = async item => {
        try {
          ...
          console.error("Erro ao buscar nbms!", err);
        }
      };
