<template>
  <div class="d-flex flex-column">
    ...
    >
      {{ $i18n.translate("adicionar") }}  {{ $i18n.translate("classes") }}
    </a>
  </div>
  <div class="card shadow">
    <div class="card-header">
      {{ $i18n.translate("classes") }}
    </div>
...
export default {
  setup() {
    ...
    let hColumns = ref([
      ...
      {
        key: "validadeDiasFarmaciaPopular",
        label: "validadeFarmaciaPopularDias",
      },
    ]);
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/classe/editar/" + e.item.id);
      }
    ...
    function atualizarComp() {
      ...
      componentePaginacao.value = false; //isso não ta resolvendo
      servico.getGridPage(page).then(response => {
        console.log("response atualiza componete", response)
        componentePaginacao.value = response; //isso não ta resolvendo
         items.value = response.itens;
        // console.log("atualiza componente " , response.itens)
      });
    }
    onMounted(async () => {
