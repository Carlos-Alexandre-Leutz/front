<template>
  <div class="d-flex flex-column">
    <a
      ...
      href="/#/listaControlado/novo"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("listaControlado") }}
    </a>
  </div>
      ...
      <suspense>
        <template #default>
          <Table
            ...
            pathPaginaAtual="listaControlado"
            action="true"
...
export default {
  setup() {
    ...
    let hColumns = ref([
      ...
      {
        key: "numeroReceitaObrigatorio",
        label: "numeroReceitaObrigatorio",
      },
    ]);
   ...
   const lang = () => {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/listaControlado/editar/" + e.item.id);
      }
    ...
    onMounted(async () => {
      ...
      const onDelete = async item => {
        try {
          ...
          console.error("Erro ao buscar Lista Controlado!", err);
        }
      };
