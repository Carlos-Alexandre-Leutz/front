<template>
  <div>
    <form @submit="validarForm" id="form">
          ...
          <div class="card shadow">
            <header class="card-header">
              <strong class="align-self-center">
                {{ $i18n.translate("cadastroGrupoUsuario") }}
              </strong>
            </header>
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                  <div class="form-group">
                    ...
                    <textarea
                      ...
                      class="form-control"
                      :placeholder="$i18n.translate('digiteDescricao')"
                      maxlength="255"
                      required
...
import servico from "../../../services/usuario/grupoUsuario.js";
import traducaoAlertas from "../../../plugins/i18n/alertas";
import Swal from "sweetalert2";
export default {
  setup() {
    const {
      params: { grupoUsuarioId, visualizar },
    } = useRoute();
    ...
    const validarForm = e => {
      ...
      if (
        ...
        servico.edit(viewModel.value).then(response => {
          if (response.status == 200) {
            ...
            Swal.fire({
              icon: "warning",
              title: `${lengBrEs.erro}`,
              text: `${lengBrEs.erro} ${response.status} ${lengBrEs.oItem} "${viewModel.value.descricao}"  ${lengBrEs.naoFoiEditado}.`,
            });
        ...
        servico.add(viewModel.value).then(response => {
          let itenSelect = reduzDescricao(viewModel.value.descricao);
          console.log(itenSelect);
          if (response.status == 200) {
            Swal.fire({
              icon: "success",
              title: `${lengBrEs.concluido}`,
              ...
              text: `${lengBrEs.oItem} "${itenSelect}" ${lengBrEs.foiAdicionado}.`,
            }).then(result => {
              if (result.isConfirmed) {
                console.log("continuar cadastrando");
              } else if (result.isDenied) {
                router.push("/grupoUsuario/1");
              }
            });
            limpaForm();
          } else {
            Swal.fire({
              icon: "warning",
              title: `${lengBrEs.erro}`,
              text: `${lengBrEs.erro} ${response.status} ${lengBrEs.oItem} ${viewModel.value.descricao} ${lengBrEs.naoFoiAdicionado}.`,
            });
            document.getElementById("descricao").focus();
          }
        });
    ...
    //script referente a atalhos => melhorar é precisso
