<template>
  <div class="d-flex flex-column">
    <a
      ...
      href="/#/metodo/novo"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("metodo") }}
    </a>
...
export default {
  setup() {
    ...
    let hColumns = ref([
      { key: "descricaoMetodo", label: "descricao" },
      { key: "gotasMetodos", label: "gotasMetodos" },
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/metodo/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let descricaoItemSelected = e.item.descricaoMetodo;
        Swal.fire({
          icon: "question",
