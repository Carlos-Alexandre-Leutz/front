        ...
        <Suspense>
          <template #default>
            <Table
              ...
              pathPaginaAtual="naturezaOperacao"
              action="true"
...
export default {
  setup() {
    ...
    let hColumns = ref([
      ...
      {
        key: "observacaoNatureza",
        label: "observacaoNatureza",
      },
    ]);
    ...
    title = ` ${languagePageNow.add} ${languagePageNow.tipoContato}`;
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/naturezaOperacao/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 2) {
        console.log("Editar", e.item.id);
        router.push("/naturezaOperacao/editar/" + e.item.id + "/visualizar");
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let descricaoItemSelected = e.item.descricaoNatureza;
        Swal.fire({
          icon: "question",
