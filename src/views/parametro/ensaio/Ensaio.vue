<template>
  <div class="d-flex flex-column">
    <a
      ...
      href="/#/ensaio/novo"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("ensaio") }}
    </a>
  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/ensaio/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let descricaoItemSelected = e.item.descricaoEnsaio;
        Swal.fire({
          icon: "question",
