<template>
  <div class="d-flex flex-column">
    ...
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("cotacao") }}
    </a>
  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/cotacao/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let moedaCotada = e.item.descricaoMoeda;
        let moedaParaCotacao = e.item.moedaCotacao;
        Swal.fire({
          icon: "question",
          title: `${lengBrEs.desejaExcluir}`,
          confirmButtonText: `${lengBrEs.sim}`,
          text: ` ${lengBrEs.aCotacao}  "${moedaCotada} ${lengBrEs.para} ${moedaParaCotacao}"?`,
            ...
            servico.remove(itemSelected).then(response => {
              if (response.status == 200) {
                Swal.fire({
                  ...
                  text: `${lengBrEs.oItem} "${moedaCotada}" ${lengBrEs.foiExcluido} `,
                }).then(result => {
                ...
                Swal.fire({
                  icon: "warning",
                  title: `${lengBrEs.erro}`,
                  text: `${lengBrEs.erro} ${response.status} "${moedaCotada}"  ${lengBrEs.naoFoiExcluido}`,
                });
              }
            });
          }
