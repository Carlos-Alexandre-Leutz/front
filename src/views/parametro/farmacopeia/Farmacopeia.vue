...
export default {
  setup() {
    ...
    let hColumns = ref([
      {
        key: "descricaoFarmacopeia",
        label: "descricao",
      },
      {
        key: "observacaoFarmacopeia",
        label: "observacao",
      },
    ]);
    ...
    title = ` ${languagePageNow.add} ${languagePageNow.farmacopeia}`;
    const onEdit = e => {
      let lengBrEs = lang();
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/farmacopeia/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let descricaoItemSelected = e.item.descricaoFarmacopeia;
        Swal.fire({
          icon: "question",
