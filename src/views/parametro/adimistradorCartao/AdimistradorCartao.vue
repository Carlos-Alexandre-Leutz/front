        ...
        <Suspense>
          <template #default>
            <Table
              ...
              pathPaginaAtual="adimistradorCartao"
              action="true"
  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/adimistradorCartao/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        let descricaoItemSelected = e.item.nomeAdministradora;
        Swal.fire({
          icon: "question",
