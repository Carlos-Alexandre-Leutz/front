<template>
  <div class="d-flex flex-column">
    <a
      ...
      href="/#/contabilista/novo"
    >
      {{ $i18n.translate("adicionar") }} {{ $i18n.translate("contabilista") }}
    </a>
  ...
  setup() {
    ...
    const onEdit = e => {
      ...
      if (e.target.dataset.edit == 1) {
        ...
        router.push("/contabilista/editar/" + e.item.id);
      }
      if (e.target.dataset.edit == 0) {
        itemSelected = e.item.id;
        console.log(e.item);
        let descricaoItemSelected = e.item.nmContabilista;
        Swal.fire({
          icon: "question",
